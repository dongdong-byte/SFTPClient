...
### MVP2 현재 실행 범위 (2026-09-15 확정, 2026-09-20 DOWNLOAD 편입)

> 세트 원자성의 확정 결정은 아래 「세트 완성도 게이트 — 현행 결정 통합」에서
> 확인한다. 원문의 상세 근거·번복 이력·예시는 본 문서 12절에 보존되어 있다.
...
### 세트 완성도 게이트 — 현행 결정 통합 (2026-09-21)

출처: [SFTPClient_MVP2_CONFIRMED_DECISIONS_v2.md](SFTPClient_MVP2_CONFIRMED_DECISIONS_v2.md).
원본 파일은 수정·삭제하지 않는다. 원문 1~19절은 이미 본 문서 12절에
통합되어 있으므로 중복 복사하지 않고, 아래에 현재 적용할 결정을 정리한다.
원문의 “확정 + 구현 완료”는 세트 게이트 구현을 뜻하며 resend·download까지
구현됐다는 뜻이 아니다. 이후 범위 변경과 유닛 1~5 결정을 함께 적용한다.

**명령과 책임**

- `Recover`는 IN_PROGRESS 크래시 회수, `resend`는 운영자 지정 로컬→원격
...
- 원자성은 **전송 시작 조건**이다. 필수 kind가 모두 관측되어야 그 세트의
  후보가 전송을 시작한다. 완료 원자성·원격 롤백·일괄 삭제는 구현하지 않는다.
- 게이트 기본값은 OFF이고 기관별 정책에 따라 opt-in한다. 미완성 세트는
  선택 종도 함께 보류한다. 완성 판정에 참여하지 않는 종류라도 완성 세트에서는
  기존 후보 규칙에 따라 전송한다. OptionalKinds 설정은 만들지 않는다.
- 완성 판정의 재료는 후보 목록이 아닌 유효한 `observed` 파일이다.
  이미 VERIFIED인 멤버도 센다. Ingress 거부, 임시 파일, 해시 불안정·가드
  불일치로 보류한 파일은 존재 근거로 쓰지 않는다. 재시도 후보에도 게이트를 적용한다.
- 세트 소속을 파싱할 수 없는 파일은 개별 통과시키고 `SetUnparsed`·WARN으로
  관측한다. 미지원 Category 오류와 파싱 유보를 합쳐 조용히 우회하지 않는다.
- `MaxFilesPerRun`은 게이트 ON에서 category+SetKey의 세트 경계로 절단한다.
  들어가지 못하는 세트는 통째로 넘기며, 절단 후 빈자리를 뒤 후보로 채우지 않는다.
  게이트 OFF는 기존 파일 단위 절단이다.
- 관측 사실은 common_ledger에 기록하되, 보류 때문에 새 PENDING을 등록하지 않는다.
  기존 전송 이력은 유지한다. COMPLETE/INCOMPLETE/HELD 상태는 새로 영속화하지 않는다.

**버전·파일명 파싱·설정**

- 버전의 유일한 출처는 닫힌 열거형 `Category.RinexVersion()`이다.
  카테고리 이름 문자열 파싱이나 config RinexVersion 키를 추가하지 않는다.
  Categories 전수 매핑 테스트와 default error를 유지한다.
- RINEX2 set_key는 세션 문자까지 포함한다. RINEX3/4는 필드를 해석해
  관측소·소스·시각·기간으로 구성하며 레이트 필드가 없는 항법 이름도 처리한다.
  뒤에서 고정 개수의 문자열을 잘라 키를 만드는 방식은 쓰지 않는다.
- kind는 데이터 종류(o/mo/mn 등)이며 표현 형식(rnx/crx)·압축(gz)은 제외한다.
  필수 종류를 버전별 상수로 강제하지 않는다. RINEX3/4 파싱 지원은 유지한다.
- 정책은 버전별 `[SET.RINEXx] RequiredKinds` 한 곳에 둔다. false는 OFF,
  CSV는 필수 종류 목록이다. 별도 Enabled·OptionalKinds·Daily/Hourly 중복 설정은 없다.
- 섹션 부재는 OFF다. 섹션이 있으면 RequiredKinds 키가 필수이며 빈 값·true·
  빈 항목·중복·영문 외 문자는 오류다. 이 최종 구현 규칙이 원문 §4의 초기
  “부재 동작 별도 규정 없음”보다 우선한다. SET 섹션 목록은 Categories에서 파생한다.

**Ledger와 역사 기록의 경계**

- set_key/kind는 NOT NULL DEFAULT ''와 소문자 CHECK를 유지한다.
  파생은 UpsertCommon 내부의 SetKeyKind가 맡고 호출자의 임의 입력으로 바꾸지 않는다.
  빈 값은 세트 소속 미확정이다. 기존 행은 마이그레이션에서 파생 값을 백필한다.
- 원문의 v4→v5 자동 전환은 당시 구현 기록이다. 현재는 유닛 2의 schema v6 및
  v4→v5→v6·v5→v6 경로가 기준이며, identity 검사와 트랜잭션 경계를 유지한다.
- 원문에 있는 일일 미완성 세트 리포트, 게이트 활성화 운영 절차,
  RINEX3 운영 프로파일은 이후 결정으로 현재 산출물에서 제외됐다.
  MaxHoldDays와 Linux 추가 보안도 이 통합을 근거로 새로 구현하지 않는다.
- 원문 §18의 과거 우선순위·MVP 번호는 아래 미결 목록과 상단 현행 범위를
  대체하지 않는다. 상세 채택·기각 이유는 12절의 해당 번호를 참조한다.
...
│   │                           HourLayout·(HH) 과도기 검증은 경로 범용화로 제거되었다
│   │                           (PATH_DESIGN v3). 잔존 키·토큰은 알 수 없는 키/토큰으로 거부된다.
...
### 9.3 Hourly Scan 의 `(HH)` 계산 — 제거 완료 (2026-09-22, PATH_DESIGN v3)

> **완료 기록.** 아래 지시대로 `HourLayout`·`(HH)` 24회 계산을 제거하고 날짜
> 폴더 아래 재귀 탐색으로 교체했다. 확정 설계·기각 이력(깊이 제한, 파일명
> 추출 연기, PathMarker 예비 등)은 `docs/path/SFTPClient_PATH_DESIGN_v3.md`
> §1~§2가 갖는다. "하지 말 것" 문단은 재론 방지를 위해 그대로 유지한다.
> 원문(구현 전 지시)은 기록으로 남긴다:
...
| DOWNLOAD | **현재 알려진 모든 기관에 불필요.** 원본 서버와 중간 서버 양쪽에 설치할 수 있으면 다단 구성도 PUT 두 번으로 성립한다. "타 기관 소유라 설치가 불가한 서버" 가 나타날 때를 위한 빈 칸이다. **→ 2026-09-20 번복:** 기관별 제각각인 저장 경로를 표준 폴더로 흡수하는 **로컬 수신**으로 의미를 재정의해 MVP2에 편입했다(상단 「MVP2 현재 실행 범위」 6번). 원격 수신에 대한 위 판단은 유지한다 |
...
### 18. 잔여 항목 (2026-09-15 갱신, 2026-09-23 범위 반영)

- ~~v5 Ledger 마이그레이션 방법~~ → **해결**: Open 시 자동 단일 스텝
  (v4→v5, 트랜잭션 ALTER+백필+버전 갱신). §19-3.
- **MVP2 남은 목표 1:** ~~`resend` 인자 설계·구현 및 세트 게이트 우회~~ → **완료 (2026-09-22).**
