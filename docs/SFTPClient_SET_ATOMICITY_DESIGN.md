...
## 8. 게이트 활성화 절차 (운영 결정 — 1.3절)

전제: resend 배포 + 해당 서버의 미완성 세트 해소 경로 +
**일일 리포트 가동**(보류 관측 수단 없이 ON 금지 — 필수 승격).

1. 백필된 원장에서 실측: `SELECT kind, COUNT(*) FROM common_ledger
   WHERE category=? GROUP BY kind;`
2. 실측 분포 그대로 RequiredKinds 작성 (없는 종을 적으면 전송 전면 보류)
3. 배포 직후 수동 1회 실행으로 [SET]/[LEDGER] 로그 육안 확인
