...
**임시 접미사** — `scan`은 `.part`·`.filepart`를 거르지 않는다. 관측 사실만 올린다.
후보에서 제외하는 것은 `verify`(Ingress)가 `IsPartFile`로 한다.
다른 접미사가 쓰이더라도 mtime이 최근이면 Grace Time에 걸린다.

방어선은 접미사마다 다르다. `.part`는 `NormalizeName`이 접미사를 떼므로
장부 이름 가드(`NormalizeName(name) != name`)가 2차로 거부한다.
`.filepart`는 정규화 불변이라 그 가드가 통과한다. 미완성 입력 제외는
`IsPartFile` 게이트에만 의존한다(UNIT5). 스캔을 거치지 않는 쓰기 경로를
추가할 때 이 비대칭을 유지해야 한다.
