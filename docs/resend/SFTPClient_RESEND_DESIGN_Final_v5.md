...
### v4 보강 (2026-09-21, 구현 착수 전)

| 항목 | v4 최초 | v4 보강 |
|---|---|---|
| 자동 창 하한 | Retention 한계 | **max(Retention 한계, 장부 운영 시작일)** (§3.5) |
| 전송 순서 | 언급 없음 | ② 안에서도 **관측일이 오래된 것부터** (기존 put 정렬 재사용, §6.2) |
| 구현 계획 | 7개 | 커밋 분할 재구성 + `ledger: 운영 시작일` 추가 (§10) |

**보강 사유:** seed는 설치 시 ScanDays 창만 장부에 채운다. 설치일보다 오래된 날짜의
파일은 기존 송신 수단(WinSCP 스크립트 등)이 이미 보냈지만 장부에 행이 없다.
자동 창 하한이 Retention 한계뿐이면, resend 배포 첫 정시 회차가 이 파일들을 전부
신규로 판정해 대량 재전송한다(지리원 규모 약 26일 × 하루 약 5000개). 9.9절에서
디렉터리 mtime 안을 기각한 것과 같은 사고다. 새 설치처뿐 아니라 운영 기간이
Retention 한계보다 짧은 기존 현장에도 해당한다.

### v4 구현 완료 (2026-09-22, 커밋 1~8)

§10 의 커밋 1~8 이 전부 구현·검증되었다 (`go test ./... -count=1` 전 패키지
통과, 운영 시나리오 13종은 `cmd/rinexclient/autoresend_operations_test.go`).
**구현과 이 문서가 다르면 코드가 사실의 주인이다.** 구현이 본문과 다르게
확정한 지점과, 구현 중 이 문서 밖에서 닫힌 결정은 다음과 같다.

| 항목 | 본문 | 구현 확정 |
|---|---|---|
| §3.5 origin 기록 | "새 DB: 장부를 처음 만들 때 기록" | **빈 DB 는 기록하지 않는다.** 행이 생긴 뒤 `MIN(first_seen)` 날짜로 기록하고, 조회(`OperationOrigin(ctx, now)`)는 키→행 MIN→오늘 순 지연 계산. 설치 당일 dry-run 날짜가 origin 으로 굳는 것을 막는다 (`internal/ledger/origin.go`) |
| §6.1 ② 생략 조건 | stall·①의 오류 | + **①의 파일 단위 전실패**(`failed > 0 && verified == 0`) 시 `[RESEND][WARN] skipped (put error…)` 후 생략, 종료 코드 0. 일부 성공이면 ② 진행 — 메시지 문서 MSG-XFER-01 의 "② 차단 여부"를 이 조건으로 닫았다 |
| §6.1 단계 표식 | "요약 한 줄" | ② 시작에 `[RESEND] auto from=… to=… budget=N` (MaxFilesPerRun=0 은 `budget=unlimited`) — 순서·예산의 관측 근거 |
...
ScanDays + 2` 는 양끝 포함 **1일짜리 유효 창**이며 본문 §3.3 이 이미 그렇게
적혀 있다 (초기 배포판의 "이하" 서술은 폐기).
...
| **자동 resend** | ScanDays ~ max(Retention 한계, 운영 시작일) | **§3.3 — 창 계산이 한계를 넘지 않음, §3.5 — 운영 시작일 이전 제외** |
...
origin = 장부 운영 시작일 (UTC 날짜, §3.5)

자동 resend 창:  From = max(limit, origin)
...
### 3.5 자동 창 하한 — 장부 운영 시작일 **[확정]**

**문제.** §3.2는 "행이 **지워진** 파일"을 막는다. 그런데 행이 **처음부터 없는**
파일도 있다. 이 장부가 운영을 시작하기 전의 날짜는 한 번도 스캔된 적이 없고,
그 파일들은 기존 송신 수단이 이미 보냈을 수 있다. seed도 설치 시 ScanDays 창만
채운다. 자동 창이 이 날짜들을 훑으면 전부 신규로 판정되어 대량 재전송된다.

**결정.** 자동 resend 창의 하한을 `max(Retention 한계, 운영 시작일)`로 둔다.

- **운영 시작일(origin)**은 이 장부가 처음 운영을 시작한 UTC 날짜다.
  - **커밋 4 구현 보완:** DB 파일 생성과 실제 운영 시작을 구분한다. 새 DB·기존 DB
    모두 값이 없고 행이 있으면 `MIN(common_ledger.first_seen)`의 UTC 날짜로 채운다.
    빈 DB에서는 기록하지 않고 조회 시 회차의 UTC 오늘을 반환한다.
  - 빈 DB에 첫 행이 생긴 회차에서는 조회가 그 행의 `first_seen` 날짜를 반환하며,
    다음 Open에서 영구 기록한다. 조회 함수 자체는 쓰지 않는다.
    설치 때 dry-run만 하고 몇 주 뒤 운영을 시작해도 설치 전 점검 날짜를 하한으로
    과거 자료가 대량 선택되는 일을 막기 위한 보완이다.
  - 저장된 값의 날짜 형식이 잘못되면 Open·조회 모두 오류로 중단하며,
    오늘이나 Retention 한계로 대체하지 않는다.
  - 한 번 기록하면 바꾸지 않는다. 보관 위치는 `schema_meta`의 키 하나로 하고
    `schema_version`은 올리지 않는다(구현 시 확정).
- **origin 당일부터** 포함한다. 첫 회차는 origin 이전 ScanDays 창도 스캔했지만,
  그 구간의 늦은 유입까지 자동으로 잡으려고 하한을 앞당기지는 않는다. 보수적으로
  둔다. 필요한 경우 수동 resend로 보낸다.
- **수동 resend에는 적용하지 않는다.** 운영자가 기간을 명시한 것은 "그 기간을 보내라"는
  지시이며, 운영 시작 전 누락분을 보내는 정규 경로가 수동 resend다. 수동 요청이
  origin 이전 날짜를 포함하면 시작 로그에 `before_origin=` 일수를 남긴다. 거부하지 않는다.
- origin이 limit보다 과거가 되면(운영 기간이 Retention 한계보다 길어지면) 하한은
  자연히 limit가 된다. 이 규칙은 운영 초기에만 실제로 작동한다.
- 자동 창이 비는 경우(`From > To`)는 §3.3과 같이 자동 단계를 건너뛴다.
  운영 시작 후 ScanDays가 지나기 전에는 항상 이 경우다. 이때는 WARN이 아니라
  INFO로 남긴다(정상 상태이므로).
...
- **전송 순서 [확정]:** 단계 간에는 ①(최근 자료)이 먼저다. ② 안에서는 기존
  `SortCandidates`(관측일 오름차순 → 파일명 → 카테고리)를 그대로 써서 **관측일이
  오래된 것부터** 보낸다. 예: 8·9·10일 전 자료가 함께 들어오면 10일 전 → 9일 전 →
  8일 전 순이다. 오래된 날일수록 자동 창 하한(Retention 한계)에 가까워 곧 창 밖으로
  나가므로, 예산이 모자랄 때 먼저 보내야 누락이 줄어든다. 새 정렬 규칙을 만들지 않는다.
...
한 번에 하나씩 진행한다. 각 구현 커밋에 해당 테스트를 함께 넣고, 자동·수동
연결 단계(7·8번)에서 관련 회귀 검증을 수행한다. 뒤 단계에서만 쓰는 편의 코드를
앞 커밋에 미리 넣지 않는다. 하위 계층(3~6번)은 호출처가 7·8번에서 생기지만
각 커밋의 테스트로 동작을 고정한다.
...
| 9 | docs: 구현 결과·운영 안내 정합 | GUIDELINES(9.9절 대체 포함) 관련 절, 설정 예시(RetentionDays 35·§6.4 운영 계약), 실행 예시, 검증 결과를 실제 구현에 맞춘다. 새 기능은 넣지 않는다 | — |

기본 9개다. 7번 직전에 동작 변경 없는 `run()` 함수 추출이 필요하면 별도 커밋으로
두어 10개가 된다. 고정 개수는 아니다.

**구현 주의 — 6번 소진 파일 재시도:** 후보 필터에서 소진 파일을 올려도
`ledger.BeginPut`이 `WHERE ... AND attempts < ?`로 막아 `ErrNotCandidate`가 되고,
전송은 이를 조용히 건너뛴다(NotCandidate 카운트만 증가). ledger는 바꾸지 않고,
재시도 후보에는 `maxRetries` 인자로 "현재 attempts + 1"을 넘긴다(한 번 실행에 한 번
시도). 이 때문에 6번은 `transfer.go`를 포함하며, 재시도 후보는 `IsRetry=true`로 두어
`transfer.go`의 "FAILED 행인 후보는 IsRetry뿐" 불변식을 지킨다. 테스트는 필터가 아니라
**실제 BeginPut을 거쳐 VERIFIED가 되는지**로 확인한다.
...
| Q16 | ② 전송 순서 = 관측일 오래된 것부터, 기존 정렬 재사용 (§6.2) | **확정** (v4 보강) |
