...
**0바이트·부분 기록 파일은 여전히 관측된다. 다만 원인이 다르다 (v5 정정).**

v4 문서는 "수집 프로그램이 파일을 먼저 만들고 내용을 이어 쓰는" append 형을 전제했다.
**우리 서버에는 그 전제가 적용되지 않는다.**

| | 무엇이 일어나는가 | 지속 시간 |
|---|---|---|
| BNC / 수신기 자체 | 관측하며 append | 1시간 ~ 하루 |
| **우리 서버** | 수신기가 **QC 를 마친 완제품**을 밀어 넣는 중 | **수 초 ~ 수 분** |
...
**정확성의 주인은 여전히 `revision`이다. 그리고 근거가 더 강해졌다.**
...
전송 시작 → 중간에 네트워크 끊김 → 절반짜리 파일이 남음
  ...
  → 수신기가 재시도하여 완전한 파일로 덮어씀
  → size 변경 감지 → revision +1 → 재전송              ← 회수
```
...
**v4 초안의 전제 두 개가 모두 사실이 아니었음을 기록한다.**

| 초안의 전제 | 실제 |
|---|---|
| "파일이 1시간 넘게 쓰이는 일은 없다" | append 형이라면 Daily 는 하루 종일 쓰인다 |
| "현장 파일 생성이 append 형이다" | **우리 서버에는 QC 완제품만 들어온다** (4.4) |

두 번째가 v5 에서 밝혀지면서 첫 번째 문제도 함께 사라졌다.
**결론(Stability Check 기각)은 유지되지만 근거는 두 번 바뀌었다.**
...
| Grace Time | 작성 중 파일 배제 장치 | **작동한다. 우리 서버에는 QC 완제품만 유입** | 4.4 |
